...
class Search extends Component {
  ...
  getDataSearch = _.debounce(() => {
    ...
    const users = firebase.database().ref('users/profiles').on('child_added', (res) => {
      let data = res.val();
      let fullData = res.val();
  ...
  _renderItem = ({ item, index }) => {
    ...
    return (
      <TouchableOpacity style={{ flexDirection: 'row', alignItems: 'center' }} onPress={() => this.props.navigation.navigate('Profile01', item)}>
        <ListItem
          key={index}
